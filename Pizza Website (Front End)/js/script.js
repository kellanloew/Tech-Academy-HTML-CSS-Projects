...
function order(){
	if (clickTimes == 0){
		...
		var cheesers = $('input[type=radio][name=cheese]:checked').attr('id'); // Stores cheese from the id of the selected radio button
		document.getElementById(cheesers+"-order").style.display = "inline";
		document.getElementById(cheesers+"-order").innerHTML += "<br>";
		
		var cheesePrice = cheeseCalc(cheesers);
		...
		for(j = 0; j < selectedVeggies.length; j++){
			...
			else if(selectedVeggies[j] == "jalepeno"){
				document.getElementById("jalepeno-order").style.display = "inline";
				document.getElementById("jalepeno-order").innerHTML += "<br>";
				if(j == 0){
					document.getElementById("jalepeno-total").innerHTML += '$' + 0;
				}
				else{
					document.getElementById("jalepeno-total").innerHTML += '$' + veggiePrice;
				}
			}
			totalVeggiePrice = j;
...
//CANCEL Function
function cancel(){
	document.getElementById("order-box").style.opacity = "0"; //Hides order box
	
	$(".price").empty();	//Deletes total from previous order
